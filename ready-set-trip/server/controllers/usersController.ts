...
const usersController: UsersController = {
  async incrementHabit(req, res, next) {
    ...
    try {
      ...
      const result = await db.query(
        `UPDATE users SET ${habit}_count = ${habit}_count + 1 WHERE id = $1 RETURNING ${habit}_count`,
        [userId]
      );
  ...
  async setUserTrip(req, res, next) {
    ...
    try {
      const response = await db.query(`UPDATE users SET trip_id = $1 WHERE id = $2`, [tripTableId, userId])
      return next();
    } catch (error) {
      return next({
