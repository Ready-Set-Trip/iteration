...
const loginController: LoginController = {
  async verifyUser(req, res, next) {
    ...
    try {
      const response = await db.query('SELECT password, name, id, trip_id FROM users WHERE email = $1', [email]);
      ...
      const storedPass = response.rows[0].password;
      ...
      res.locals.tripId = response.rows[0].trip_id;
